(ns elastic.main
  "Kotodama WASM entrypoint for the Elastic clean-room actor (L5) — Clojure port.
  ...
  facts (`elastic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "elastic")
(def tier "L5")
...
(def entity-specs
  [{:entity "ClusterHealth" :plural "clusterhealths" :id-prefix "elastic_clu"
    :fields [:clusterName :status :numberOfNodes :numberOfDataNodes :activePrimaryShards :activeShards :relocatingShards :initializingShards :unassignedShards :timedOut]
    :required [:clusterName :status]
    :coerce {:numberOfNodes :int :numberOfDataNodes :int :activePrimaryShards :int :activeShards :int :relocatingShards :int :initializingShards :int :unassignedShards :int :timedOut :bool}
    :refs {}}
   {:entity "Index" :plural "indexes" :id-prefix "elastic_ind"
    :fields [:name :health :status :uuid :numberOfShards :numberOfReplicas :docsCount :docsDeleted :storeSize]
    :required [:name :health]
    :coerce {:numberOfShards :int :numberOfReplicas :int :docsCount :int :docsDeleted :int}
    :refs {}}
   {:entity "Shard" :plural "shards" :id-prefix "elastic_sha"
    :fields [:index :shardNumber :primaryOrReplica :state :docsCount :nodeName :nodeId]
    :required [:index :shardNumber]
    :coerce {:shardNumber :int :docsCount :int}
    :refs {:nodeId "Node"}}
   {:entity "Node" :plural "nodes" :id-prefix "elastic_nod"
    :fields [:ip :port :name :version :heapPercent :ramPercent :diskUsedPercent :master]
    :required [:ip]
    :coerce {:port :int :heapPercent :int :ramPercent :int :diskUsedPercent :int}
    :refs {}}
   {:entity "Alias" :plural "aliases" :id-prefix "elastic_ali"
    :fields [:alias :index :routingIndex :routingSearch :isWriteIndex]
    :required [:alias :index]
    :coerce {}
    :refs {}}
   {:entity "Document" :plural "documents" :id-prefix "elastic_doc"
    :fields [:index :version :seqNo :primaryTerm :found :routing]
    :required [:index]
    :coerce {:version :int :seqNo :int :primaryTerm :int :found :bool}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"elastic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "elastic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :elastic-compat/ready)
